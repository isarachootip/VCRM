...
export const Sidebar: React.FC<SidebarProps> = ({ currentBoardId, onSelectBoard }) => {
  const [isWorkspaceOpen, setIsWorkspaceOpen] = useState(true);

  const boards = [
    { id: 'board-5030723273', name: 'Deals & Sales Pipeline', icon: Briefcase, color: 'text-blue-500' },
    { id: 'board-leads', name: 'Inbound Leads 2026', icon: Users, color: 'text-amber-500' },
    { id: 'board-accounts', name: 'Key Enterprise Accounts', icon: Layers, color: 'text-purple-500' },
    { id: 'board-growth', name: 'Sales Forecast & Targets', icon: TrendingUp, color: 'text-emerald-500' },
  ];
    ...
    <div className="flex h-screen select-none bg-white border-r border-[#e6e9ef] shrink-0">
      ...
      <div className="w-[52px] bg-[#292f4c] flex flex-col items-center justify-between py-3 text-gray-300">
        <div className="flex flex-col items-center gap-4 w-full">
          {/* Monday Logo */}
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-rose-500 via-amber-400 to-emerald-400 flex items-center justify-center text-white font-black text-sm shadow-md cursor-pointer hover:scale-105 transition-transform" title="Monday.com Work OS">
            M
          </div>
      ...
      <div className="w-[230px] flex flex-col justify-between bg-white text-[#323338] border-r border-[#e6e9ef]">
        <div>
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-2.5 px-2.5 py-2 rounded-md text-xs cursor-pointer transition-colors ${
                        isActive 
                          ? 'bg-[#e5f2ff] text-[#0073ea] font-medium' 
                          : 'text-[#323338] hover:bg-[#f5f6f8]'
                      }`}
                    >
                      <Table size={15} className={isActive ? 'text-[#0073ea]' : 'text-gray-400'} />
        ...
        <div className="p-3 border-t border-[#e6e9ef] flex items-center justify-between text-xs text-gray-500">
          ...
          <span className="text-[10px] bg-blue-50 text-[#0073ea] px-1.5 py-0.5 rounded font-semibold">
            Pro CRM
          </span>
        </div>
      </div>

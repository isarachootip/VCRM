...
export const StatusPicker: React.FC<StatusPickerProps> = ({ currentStatus, onChange }) => {
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center" ref={dropdownRef}>
      ...
      {isOpen && (
        <div 
          className="absolute z-50 top-full left-0 mt-1 w-44 bg-popover text-popover-foreground rounded-xl shadow-xl border border-border p-1.5 grid grid-cols-1 gap-1 animate-in fade-in zoom-in-95 duration-100"
          ...
          {statuses.map((st) => {
            ...
            return (
              <button
                ...
                className={`w-full py-1.5 px-2.5 rounded-lg text-xs font-medium text-white flex items-center justify-between transition-all hover:opacity-90 ${
